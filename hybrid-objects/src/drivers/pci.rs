...
impl PortOps for PortOpsImpl {
    ...
    unsafe fn write8(&self, port: u16, val: u8) {
        my_x86_64::out8(port, val);
    }
    unsafe fn write16(&self, port: u16, val: u16) {
        my_x86_64::out16(port, val);
    }
    unsafe fn write32(&self, port: u32, val: u32) {
        my_x86_64::out32(port, val);
    }
}
...
unsafe fn enable(loc: Location) {
    ...
    let orig = unsafe { am.read16(ops, loc, PCI_COMMAND) };
    ...
    let mut cap_ptr = unsafe { am.read8(ops, loc, PCI_CAP_PTR) as u16 };
    while cap_ptr > 0 {
        let cap_id = unsafe { am.read8(ops, loc, cap_ptr) };
        if cap_id == PCI_CAP_ID_MSI {
            let orig_ctrl = unsafe { am.read32(ops, loc, cap_ptr + PCI_MSI_CTRL_CAP) };
            ...
            if (orig_ctrl >> 16) & (1 << 7) != 0 {
                // 64bit
                unsafe { am.write32(ops, loc, cap_ptr + PCI_MSI_DATA_64, irq + 32) };
            } else {
                // 32bit
                unsafe {
            ...
            unsafe { am.write32(ops, loc, cap_ptr + PCI_MSI_CTRL_CAP, orig_ctrl | 0x10000) };
            msi_found = true;
        }
        cap_ptr = unsafe { am.read8(ops, loc, cap_ptr + 1) as u16 }
    }
