...
use crate::app::loader::get_app_name;
use crate::trap::{set_user_rsp, syscall_return, CallerRegs};
use core::ops::Deref;
...
impl AppManager {
    ...
    pub unsafe fn load_next_app(&self) {
        ...
        serial_println!("[kernel] Loading app_{} {}", app_id, get_app_name(app_id));
...
pub fn run_next_app() -> ! {
    unsafe {
        ...
        let ctx: &mut CallerRegs = &mut *((KERNEL_STACK.as_ptr_range().end as usize
            - core::mem::size_of::<CallerRegs>())
            as *mut CallerRegs);
        ctx.rcx = APP_BASE_ADDRESS as _;
        // ctx.rcx = ((&_app_count as *const u64).add(2)) as _;
        ctx.r11 = 0;
